...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads=12, mlp_ratio=4.,
        ...
        self.dense = nn.Linear(56, 56 * 56)
        self.dense2 = nn.Linear(384, 384 * 197)
    ...
    def forward(self, x, x_t):
        ...
        # 全局平均池化
        # x = F.adaptive_avg_pool1d(x, 1)  # [32, 256, 56, 1]
        # print(x.shape)
        # # 压缩维度以适应全连接层
        # x = x.squeeze(-1)  # [32, 256, 56]
        # print(x.shape)
        # # 通过全连接层
        # x = self.dense(x)  # [32, 256, 56*56]
        #
        # # 重新调整张量的形状以匹配原始形状
        # x = x.view(-1, 256, 56, 56)  # [32, 256, 56, 56]

        x_t = x_t.transpose(1, 2)  # [32, 384, 197]

        # 对每个特征应用全局平均池化
        x_t = F.adaptive_avg_pool1d(x_t, 1)  # [32, 384, 1]

        # 压缩维度以适应全连接层
        x_t = x_t.squeeze(-1)  # [32, 384]

        # 通过全连接层
        x_t = self.dense2(x_t)  # [32, 384*197]

        # 重新调整张量的形状以匹配原始形状
        x_t = x_t.view(-1, 197, 384)  # [32, 197, 384]
...
class Conformer(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_cls = torch.sigmoid(conv_cls)
        tran_cls = torch.sigmoid(tran_cls)
