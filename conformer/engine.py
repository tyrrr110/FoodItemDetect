...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            # output shape: (2, batch size, n_classes)
        ...
        if isinstance(outputs, list):
            ...
            wandb.log({"Batch Loss0": loss_list[0].item()})
            wandb.log({"Batch Loss1": loss_list[1].item()})
        else:
            metric_logger.update(loss=loss_value)
            wandb.log({"Batch Loss": loss_value})
...
def evaluate(data_loader, model, device, wandb):
    ...
    for images, target in metric_logger.log_every(data_loader, 100, header):
        ...
        if isinstance(output, list):
            # Conformer
            acc1_head1 = accuracy_food(output[0], target)
            acc1_head2 = accuracy_food(output[1], target)
            acc1_total = accuracy_food((output[0] + output[1])/2, target)
        else:
            # others
        ...
        if isinstance(output, list):
            ...
            wandb.log({"acc0": acc1_head1.item()})
            wandb.log({"acc1": acc1_head2.item()})
            wandb.log({"acc_total": acc1_total.item()})
